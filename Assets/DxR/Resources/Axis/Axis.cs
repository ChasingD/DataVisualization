...
using SimpleJSON;
using UnityEngine;
using UnityEngine.UI; // 引入Slider组件命名空间
...
namespace DxR
{
    public class Axis : MonoBehaviour
    {
        ...
        private void EnableThresholdFilter(JSONNode axisSpecs, DxR.Scale scale)
        {
            ...
            // 使用Unity的UI.Slider组件替换原来的SliderGestureControlBothSide
            Slider sliderControl = slider.GetComponent<Slider>();  // 使用UI的Slider组件
            if (sliderControl == null) return;

            float domainMin = float.Parse(scale.domain[0]);
            float domainMax = float.Parse(scale.domain[1]);

            sliderControl.minValue = domainMin;   // 设置滑块的最小值
            sliderControl.maxValue = domainMax;   // 设置滑块的最大值
            ...
            {
                // 修改为调用没有参数的方法
                sliderControl.onValueChanged.AddListener((value) =>
                {
                    interactionsObject.ThresholdFilterUpdated(); // 如果ThresholdFilterUpdated不需要参数，移除value
                });
            }
        }
