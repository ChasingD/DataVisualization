...
namespace DxR
{
    ...
    public class Vis : MonoBehaviour
    {
        ...
        public GameObject guiPrefab; // Assign this in the Unity Editor
        public Transform cameraRig;
        private List<GameObject> guiInstances = new List<GameObject>();
        public void CreateNewGUI()
        {
            if (guiPrefab != null)
            {
                GameObject newGui = Instantiate(guiPrefab, cameraRig.position + new Vector3(-0.1f, 1.2f, 0.32f), cameraRig.rotation);
                GUI guiComponent = newGui.GetComponent<GUI>();
                if (guiComponent != null)
                {
                    guiComponent.Init(this);
                    guiInstances.Add(newGui);
                }
            }
            else
            {
                Debug.LogError("GUI Prefab is not assigned.");
            }
        }
        
        public void DeleteLastGUI()
        {
            if (guiInstances.Count > 0)
            {
                GameObject guiToRemove = guiInstances[guiInstances.Count - 1];
                guiInstances.Remove(guiToRemove);
                Destroy(guiToRemove);
            }
            else
            {
                Debug.Log("没有 GUI 实例可以删除。");
            }
        }
